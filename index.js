...
async function handleChat(req, res) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    res.statusCode = 200;
    return res.end(JSON.stringify({ ok: true, provider: 'openai', available: false, reason: 'OPENAI_API_KEY not set', hint: 'Defina OPENAI_API_KEY em .env e reinicie o servidor.' }));
  }
  try {
    const OpenAI = require('openai');
    const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
    ...
    return res.end(JSON.stringify({ ok: true, provider: 'openai', available: true, model: completion.model || (process.env.OPENAI_MODEL || 'gpt-4o-mini'), reply }));
  } catch (err) {
    res.statusCode = 200;
    return res.end(JSON.stringify({ ok: false, provider: 'openai', available: false, error: String(err) }));
  }
}
...
async function handleSolve(req, res) {
  ...
  if (!apiKey || !baseURL) {
    res.statusCode = 200;
    return res.end(JSON.stringify({ ok: true, provider: 'moonshot', available: false, reason: 'MOONSHOT_API_KEY/BaseURL not set', hint: 'Defina MOONSHOT_API_KEY e MOONSHOT_BASE_URL em .env e reinicie o servidor.' }));
  }
  try {
    const OpenAI = require('openai');
    ...
    const completion = await client.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: 'Você é um agente executor para tarefas complexas. Responda de forma estruturada e objetiva.' },
    ...
    const reply = (completion.choices?.[0]?.message?.content || '').trim();
    return res.end(JSON.stringify({ ok: true, provider: 'moonshot', available: true, model: completion.model || (process.env.MOONSHOT_MODEL || 'moonshot-large'), reply }));
  } catch (err) {
    res.statusCode = 200;
    return res.end(JSON.stringify({ ok: false, provider: 'moonshot', available: false, error: String(err) }));
  }
}
